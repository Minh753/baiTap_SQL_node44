...
-- Bai_tập
-- Câu 1: tìm 5 người like nhiều
SELECT u.full_name, COUNT(*) AS tong_like FROM users u
INNER JOIN like_res lr ON lr.user_id = u.user_id
GROUP BY u.user_id, u.full_name
ORDER BY tong_like DESC
LIMIT 5;

-- Câu 2: tìm 2 nhà hàng được like nhiều nhất
SELECT res.res_name, COUNT(*) AS tong_likes FROM restaurant res
INNER JOIN like_res lr ON lr.res_id = res.res_id
GROUP BY res.res_id, res.res_name
ORDER BY tong_likes DESC
LIMIT 2;

-- Câu 3: tìm người đã đặt hàng nhiều nhất
SELECT u.full_name, COUNT(*) AS tong_dat FROM users u
INNER JOIN orders od ON od.user_id = u.user_id
GROUP BY od.user_id, u.full_name
...
LEFT JOIN orders od on od.user_id = u.user_id
WHERE lr.like_res_id is NULL AND rr.rate_res_id is NULL AND od.order_id is NULL
